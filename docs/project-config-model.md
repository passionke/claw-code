...
- 保存：`PUT /v1/project/config/{proj_id}` 的 `gitSyncJson`；PUT **省略** `gitSyncJson` 时保留库内已有配置；PAT 留空则保留已存 token。
- 推送：`POST /v1/projects/{proj_id}/git/push`（先物化再 push）。
...
1. 清空 tmpfs **`/claw_host_root`**，从 PG **effective formal rev** 写出 task / jsonl / 上表 guest 列路径（**不**依赖宿主机 `proj_*` 是否刚轮询、**不**依赖 worker 镜像新旧）。
...
宿主机 **`proj_<id>/` 物化**（`apply_project_config_for_proj`）仍以 **`home/`** 为项目 home 磁盘真源；写完 `home/` 后调用 `link_claw_compat_symlinks`（`.claw/skills`、`.cursor/rules` → `home/`），供本机 claw 发现路径。
