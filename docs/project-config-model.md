...
10. **Solve `settings.json`**：网关写入 `auto_hidden_system_prompt: 1`（默认）。当存在非空 instruction 文件（如 `CLAUDE.md`）且**无** `.claw/system_prompt_scaffold.md` 时，runtime **不再**拼接 hardcoded intro/`# System`/`# Doing tasks` 段；**有** PG scaffold 文件时仍使用 scaffold。设为 `0` / `false` 可恢复 hardcoded 与 instruction 并存。
...
1. 清空 tmpfs **`/claw_host_root`**，从 PG **effective formal rev** 写出 task / jsonl / 上表 guest 列路径（**不**依赖宿主机 `ds_*` 是否刚轮询、**不**依赖 worker 镜像新旧）。
2. **不**在 guest 创建 `home/` 或 symlink；session 内只认 `.claw/`、`.cursor/rules`、根 `CLAUDE.md`。
3. **PG 物化的 project config（skills / rules / CLAUDE / settings 等）在 guest 内只读**（`chmod` + runtime `write_file` 拦截）；session 产出写 tmpfs 其它路径，readback tar **不含** `.claw/*`、`.cursor/rules`、`CLAUDE.md`。
4. `exec` 注入 **`CLAW_PROJECT_CONFIG_ROOT=/claw_host_root`**；`gateway-solve-once` 的 `cwd` 同为 `/claw_host_root`。

宿主机 **`ds_<id>/` 物化**（`apply_project_config_for_ds`）仍以 **`home/`** 为 `ds_home` 磁盘真源；写完 `home/` 后调用 `link_claw_compat_symlinks`（`.claw/skills`、`.cursor/rules` → `home/`），供本机 claw 发现路径。

**不要**在 guest 再维护 `home/` 镜像或 symlink；**不要**让模型只靠 bash 猜目录——应用 `Skill` / `claw skills list` 或读 `.claw/skills`。

实现：`project_config_apply.rs`（`build_guest_materialize_writes`、宿主机 `link_claw_compat_symlinks`）、`session_db_sync.rs`（`materialize_in`）。
