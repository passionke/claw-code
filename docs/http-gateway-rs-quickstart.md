...
- `CLAW_GATEWAY_LOG_DIR`：可选；**JSON 行**按天滚动写入该目录（文件前缀 `http-gateway`）。不设时默认 **`/var/log/claw`**（与 sandbox `CLAW_WORK_ROOT` 分离；compose 挂载 `CLAW_HOST_LOG_DIR`）。
...
- **`clawExitCode=125`**（`detail` 常为 `gateway-solve-once failed`）
  - 多为 **e2b worker 未就绪** 或 NAS 路径缺失：检查 e2b API、`/healthz` 中 `fcSandbox`/`solveBackend`；处理：`gateway.sh build` 后 `gateway.sh up`
- **`clawExitCode=1`**
