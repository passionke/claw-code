...
sequenceDiagram
    ...
    GW->>PD: acquire(session_id, proj_id, turn_id)
    PD->>PG: SELECT 续聊制品 + cc_messages
    PD->>W: run 仅 proj_home→/claw_ds + tmpfs /claw_host_root
...
| 挂 `proj_home` | **创建时** `-v /abs/path/proj_1:/claw_ds:ro`（卷在 `run` 时固定；换项目才 `rm+run`） | 同左 |
