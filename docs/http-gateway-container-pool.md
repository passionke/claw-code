...
sequenceDiagram
    ...
    participant FS as 宿主机 ds_home

    GW->>FS: ds_lock 内写 settings.json 等同现在
...
- **谁在写 `ds_home`**：仍在 **网关 + 短时 ds_lock**，保证与 `init` / MCP probe 行为一致。  
- **容器里看到的树**：推荐 **`--volume ds_home:/workspace:rw`**（v1 简单）或 **只读 + 可写 overlay**（v2 防容器改坏 canonical，实现更重）。  
...
- **并发与 `ds_home`**：多容器 **rw 挂载同一 `ds_home`** 仍可能打穿「逻辑隔离」；更强隔离应对 **每请求拷贝 workspace** 或 **每 ds 串行**（与业务约定）。  
