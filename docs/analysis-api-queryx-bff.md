...
| `has_report` | boolean | 网关 **`hasReport`**：PG 已有 live chunk（有正文）时为 true；`succeeded` 恒 true。**仅在此为 true 时** BFF/前端可开 `biz_advice_report` SSE。见 `docs/persistence-model.md` § Live report contract (locked)。 |
...
| 日期 | 说明 |
|------|------|
| 2026-05-22 | Live 报告四条契约写入 `docs/persistence-model.md` § Live report contract (locked)；`has_report` 仅表示 PG 有正文后才可 SSE |
