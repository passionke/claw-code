...
**项目 ID 命名（`proj_id` / `projId`）**：API 与路径参数以 **`projId`** / **`proj_id`** 为准；磁盘目录为 **`proj_{id}/`**（`session_home_rel` 形如 `proj_{projId}/sessions/…`）。请求体与 query 仍接受历史别名 **`dsId`** / **`ds_id`**（反序列化后等价）。响应 JSON 字段为 **`projId`**（`GET /v1/skills/{proj_id}` 列表/单条响应为 snake_case **`proj_id`**）。PostgreSQL 保留 legacy **`ds_id`** 列（migration `005_proj_id` 镜像 **`proj_id`**）。
...
- `GET /v1/tasks/{task_id}`
  ...
  - 另含 `projId`、`progressUpdatedAtMs`、**`hasReport`**、**`reportTime`**：见 [`docs/live-report-contract.md`](live-report-contract.md) §6.4。成功后的报告正文在 **`result.outputJson.message`**（非响应顶层 `outputJson`）。

- `GET /v1/sessions/{session_id}/turns/{turn_id}/tools?proj_id=<int>`
...
- `GET /v1/sessions/{session_id}/execution?proj_id=<int>`
  - 用途：按 `(sessionId, projId)` 查看当前进度快照、`progressHistory`、网关队列统计、脱敏 trace 尾（`include_trace=true` 时含更多字段）。`progressHistory` 来自 PG `solve_timing_jsonb.progressEvents`（当前 turn）。Query 接受别名 `ds_id` / `dsId` / `projId`。
...
- `GET /v1/sessions/{session_id}/turns/{turn_id}/timeline?proj_id=<int>`
  - 用途：单轮 swimlane（progress / tool / LLM 等 lane）。来自 PG `solve_timing_jsonb`。
  - 无该会话行：404

- `POST /v1/sessions/{session_id}/turns/{turn_id}/cancel?proj_id=<int>`
  - 用途：按 **`sessionId` + `turnId` + `projId`** 取消指定轮次（推荐 Admin / BFF 使用）
  ...
  - 未知 `(session_id, turn_id, proj_id)`：**404**
...
- `POST /v1/agent/feedback`
  ...
  - 请求体：`projId`（≥1；可写别名 `dsId` / `ds_id`）、`sessionId`、`turnId`（格式 `T_<32位小写hex>`）、`feedback`（`good` | `bad`）
  - 校验：`(sessionId, projId)` 须在 `gateway_sessions`；`turnId` 须属于该会话（`gateway_turns`）
  - 成功：`sessionId`、`projId`、`turnId`、`feedback`、`updatedAtMs`
  - 同一轮再次提交为覆盖更新

- `GET /v1/agent/feedback?sessionId=<id>&projId=<int>`
  - 用途：查询该会话下**已有反馈**的轮次（未操作的 `turnId` 不出现）
  - Query：`projId` 或 `proj_id`（或 legacy `dsId` / `ds_id`）
  - 成功：`{ "sessionId", "projId", "items": { "<turnId>": "good"|"bad", ... } }`
  - 未知会话：**404**

- `turnId` 签发：每次 `POST /v1/solve` / `POST /v1/solve_async` 入队或同步受理时由网关生成；响应体与 `GET /v1/tasks/{task_id}` 含 `turnId`。`POST /v1/start` 不签发。

- `GET /v1/biz_advice_report?sessionId=<id>&turnId=<T_…>&projId=<int>`
  ...
  - 查询参数：`sessionId`、`turnId`（`T_<32 hex>`）、`projId`（≥1；可写 legacy `dsId` / `ds_id`）必填；`stream` 默认 `true`
...
- `GET /v1/skills/{proj_id}`
  ...
  - 成功响应 JSON：`{ "proj_id": <int>, "skills": [ { "skill_name": "<str>", "skill_content": "<str>" }, ... ] }`（按 `skill_name` 排序）

- `GET /v1/skills/{proj_id}/{skill_name}`
  - 用途：读取单个技能的完整 `SKILL.md` 文本
  - 成功响应 JSON：`{ "proj_id": <int>, "skill_name": "<str>", "skill_content": "<str>" }`
  - 若文件不存在：返回 `404`
...
- `GET /v1/mcp/injected/{proj_id}`
  - 用途：按 DB 配置写 `proj_<id>/.claw/settings.json` 并探针；`injectedServerNames` 为当前 DB 中的 server 名（不返回 Bearer 等敏感字段）

- `DELETE /v1/mcp/injected/{proj_id}`
  - 用途：清空或按 `server_names` 删除 `project_config` 中的 MCP 条目

## Project config (PostgreSQL)

按 `projId` 在 **`project_config`** 表存储规则、MCP、**内联 `skillsJson`**、工具勾选与 **`claudeMd`**；约定见 **`docs/project-config-model.md`**。写库后物化到 `proj_<projId>/home`；`POST /v1/init` / solve 前 / 轮询在 `contentRev` 变化时刷新。**须先有 `project_config` 行**（`POST /v1/projects` 或 `PUT`）。
...
- `GET /v1/project/config/{proj_id}`
  - 用途：读取该 `projId` 的配置行
  - 无行：**404**

- `PUT /v1/project/config/{proj_id}`
...
- `POST /v1/project/config/{proj_id}/versions/commit`
...
- `DELETE /v1/project/config/{proj_id}/versions/{content_rev}`
  - 用途：**废弃**某正式版（非当前生效版）
  - 当前生效版：**409**；`__draft__`：**400**

- `PATCH /v1/project/config/{proj_id}/versions/{content_rev}`
  - 用途：更新该正式版的**备注**（`{ "note": "…" }`，空字符串表示清空）；配置快照仍不可变
  - `__draft__`：**400**

- `GET /v1/project/config/{proj_id}/versions`
  - 用途：列出正式版历史 + 若有编辑中临时版则首行 `__draft__`（`isDraft: true`）；含 `activeContentRev`、`appliedContentRev`、`draftOpen`、每项 `note` / `isActive`

- `GET /v1/project/config/{proj_id}/versions/compare?from={rev}&to={rev}`
...
- `POST /v1/project/config/{proj_id}/versions/{content_rev}/activate`
  - 用途：将**生效版本**切换为指定历史 `content_rev` 并物化到 `home/`

- **L2 条目历史**（`domain`: `rule` | `skill` | `mcp` | `claude` | `tools`；`entity_key` 需 URL 编码，`claude`/`tools` 为 `_`）
  - `GET /v1/project/config/{proj_id}/entities/{domain}/{entity_key}/versions` — 该条目追加历史列表（`entityRev`、`createdAtMs`、`note`）
...
- `POST /v1/projects`
  ...
  - 成功：同 `InitResponse`（`projId`, `workDir`, `initialized`）

- `DELETE /v1/projects/{proj_id}`
  - 用途：删除 `work_root/proj_<id>`、`project_config` 行
  - Query：`purgeSessions`（默认 `true`）是否删除该项目的 `gateway_sessions` / `gateway_turns`
  - 无此项目：**404**

- `GET /v1/projects/{proj_id}/sessions`
  ...
  - 响应：`{ "projId", "sessions": [ { "sessionId", "createdAtMs", "updatedAtMs", "turnCount", "previewPrompt", "clientOrigin"?, "hasBadFeedback", "hasGoodFeedback" } ], "hasMore" }`（反馈标记：该会话任一轮在 `gateway_feedback` 中出现过 `bad` / `good`）

## Project workspace

- `POST /v1/init`
  - 用途：要求已有 `project_config`，按 `content_rev` 物化到 `proj_<projId>/home`（无 PG 行 **404**）
  - 轮询：可选 `CLAW_PROJECT_CONFIG_POLL_INTERVAL_SECS`（或旧名 `CLAW_PROJECTS_GIT_DS_HOME_POLL_INTERVAL_SECS`）——仅刷新有 PG 行的项目

- `GET /v1/project/claude/{proj_id}`
  - 用途：读取 CLAUDE；优先 `project_config.claude_md`，否则磁盘 `home/CLAUDE.md`

- `POST /v1/project/claude/{proj_id}`
  - 用途：写入 `project_config.claude_md` 并物化（**不写** projects-git）
  - 请求体字段：
    - `content`：必填，写入 CLAUDE 文本
  - 落盘路径：
    - `proj_<projId>/home/CLAUDE.md`
  - 返回字段：
