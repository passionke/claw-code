...
**完整说明（唯一逻辑根、各组件本地路径、Mac podman machine 直 bind）：[`docs/fc-nas-workspace.md`](../fc-nas-workspace.md)**
...
**Session 路由（Gateway，不 bind 进沙箱）：** `proj_N/sessions/{segment}` → symlink `../workers/{workerId}`。见 [`fc-nas-workspace.md`](../fc-nas-workspace.md) §1。

**禁止：** sandbox 内 `mount.nfs4`（Firecracker 无 `CAP_SYS_ADMIN`）；`volumeMounts` / `CLAW_FC_NAS_VOLUME_NAME` legacy 已移除。

代码：`claw-fc-sandbox-client/src/nas_paths.rs`（逻辑映射）、`http-gateway-rs/src/pool/fc_nas_layout.rs`（Gateway 本机路径）。
...
| `CLAW_FC_NAS_*` / `NAS_BASE_URL` | NAS export 逻辑根（nasConfig `serverAddr` fallback） |
