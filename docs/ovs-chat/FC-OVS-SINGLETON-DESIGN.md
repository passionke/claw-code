...
| **compose 不跑 OVS** | `CLAW_OVS_BACKEND=fc` 时跳过 `openvscode-server` |
...
- 启动参数：`--server-base-path=/ovs`，`--default-folder=/claw_ws`
...
- **无** openvscode-server
- NAS：`proj_N/home` → `/claw_ds`；session → `/claw_host_root`（现有 `fc_interactive_materialize.rs`）
- 端口：**7681**
...
| 沙箱 | NAS 路径（相对 export） | 容器路径 | 时机 |
|------|-------------------------|----------|------|
| OVS | `{export}` 根 | `/claw_ws` | singleton create 一次 |
| Worker（warm） | `proj_N/home` | `/claw_ds` | warm_one bake |
| Worker（acquire） | `proj_N/sessions/{sessionId}` | `/claw_host_root` | terminal_start / agent |

OVS 打开 project N：

```text
/claw_ws/proj_N/home     ← workspaceFolder（API 返回）
```

Worker REPL cwd（`ovs-*`）：

```text
/claw_ds                 ← 与 proj_N/home 同树
```

**对齐条件：** export 根一致；Gateway materialize 写 NAS 上 `proj_N/home/.vscode/settings.json`（非 Gateway 本地盘）。
...
2. 调 e2b `POST /sandboxes`：`templateID=claw-ovs`，metadata 见上
3. exec：NAS root mount → `/claw_ws`
4. exec：启动 openvscode-server（nohup + pidfile）
5. 缓存 handle + `ovs_base_url`；写入可选持久化（见 §8）
...
```text
ensure_warm(proj_id) → warm_one → bake /claw_ds → idle
terminal_start(ovs-N) → acquire → session mount → ttyd
stop / release → session_release_sh → 回 idle（保留 /claw_ds bake）
```
...
| `CLAW_FC_OVS_TEMPLATE` | `claw-ovs` | OVS 专用 template（≠ `CLAW_FC_TEMPLATE`） |
| `CLAW_FC_OVS_PORT` | `3000` | OVS 监听 / e2b 前缀端口 |
| `CLAW_OVS_MOUNT_ROOT` | `/claw_ws` | fc 模式 OVS 容器内根（API `workspaceFolder` 前缀） |
| `CLAW_FC_OVS_SANDBOX_TIMEOUT_SECS` | 同 worker | singleton 超时；需续期或 recreate |

**不变：**

| 变量 | 说明 |
|------|------|
| `CLAW_INTERACTIVE_BACKEND=fc` | worker 走 e2b |
| `CLAW_FC_TEMPLATE=claw-worker` | 仅 worker |
| `CLAW_FC_POOL_*` | worker 池 |
| `CLAW_FC_NAS_*` / `NAS_BASE_URL` | e2b 沙箱内 NFS |
...
| `claw-ovs` | `deploy/fc-sandbox/build-claw-ovs-selfhosted.py`（**新建**） | passionke openvscode-server + VSIX + nfs-common |
...
| **P2** | `CLAW_OVS_BACKEND=fc` compose 跳过 OVS；Playground 动态 base | `gateway.sh up` 无 openvscode-server |
| **P3** | `verify-fc-ovs-e2e.sh` 改 fc 路径；文档 / `.env.example` | E2E green |
...
|------|-------------|
| 设计 | 本文 |
| OVS template | `deploy/fc-sandbox/build-claw-ovs-selfhosted.py` |
| FC client | `claw-fc-sandbox-client`: `create_ovs_singleton_sandbox()`, `ovs_public_host(port)` |
...
| Compose | `compose-include.sh`: `CLAW_OVS_BACKEND=fc` → 不 merge OVS service |
| Playground | 读 `ovsUrl` / `/v1/claw/ovs` |
| Env | `env.selfhosted-e2b.example`, `boundaries-claw-stack.md` 表一行 |
| E2E | `verify-fc-ovs-e2e.sh`, `verify-ovs-claw-e2e.sh` |
