...
**NAS 路径 / bind 总览：** [`docs/e2b-nas-workspace.md`](../e2b-nas-workspace.md)
...
| # | 现象 | 根因（证据） | 修复 | 状态 |
|---|------|-------------|------|------|
| F1 | `mount.nfs4: multiple version options not permitted` | mount 同时写 `vers=4.2` 与 `nfsvers=4.2` | `e2b_interactive_materialize.rs` 只保留 `vers=4.2,_netdev` | FIXED |
...
| F4 | e2b 累积 **12** 个 orphan sandbox | gateway 重启丢内存池；`stop_session` 未杀沙箱；E2E 只 create 不 cleanup | `e2b-sandbox-cleanup.sh`；`shutdown_all`/`FcOvsSingleton::shutdown`；gateway SIGTERM 钩子；E2E 前后计数 + `terminal/stop` | FIXED（cleanup 已验证 12/12 kill）；shutdown 钩子 **待 VERIFIED** |
| F5 | `Could not resolve host: 3000-sbx_….10.8.0.9` | self-hosted domain=IP，子域不 DNS 解析 | `verify-e2b-ovs-e2e.sh` 用 `curl --resolve host:80:10.8.0.9` | FIXED |
| F5b | OVS「可达」误判 HTTP 200 | `--resolve` 打到 e2b **默认站** `<title>君子慎独</title>`，非 openvscode | 验收改查 title/关键字，不能只看 status | FIXED（脚本） |
| F6 | `terminal/start` → `timeout waiting for idle Strict worker` | `CLAW_INTERACTIVE_BACKEND=e2b` 仍走 per-proj `worker_profile_json` → podman | `interactive_backend_for_proj` 在 `interactive_backend_is_e2b()` 时强制 fc | FIXED |
...
| F13 | 同 proj 重复 warm worker（4 sandbox） | gateway 重启丢内存池，e2b 上旧 warm 仍在；`warm_one` 再 create | E2E 前 `e2b-sandbox-cleanup.sh` + restart gateway；长期：启动时 reconcile 或 shutdown 杀光 | OPEN |
...
./deploy/stack/lib/e2b-sandbox-cleanup.sh
...
| ID | 假设 | 怎么证伪/证实 |
|----|------|----------------|
| V-NAS | e2b `GET /health` → `hostMountRoot` + `sandboxInject:bind`；Gateway `nas_paths` + `e2b_nas_layout`；create/attach `nasConfig` | `curl http://10.8.0.9:3000/health`；create 后 `mountpoint /claw_ds`；attach 后 `mountpoint /claw_host_root` | **PARTIAL VERIFIED** 2026-06-20：`/claw_ds`、`/claw_ws` virtiofs OK；attach bind 依赖 `POST /sandboxes/{id}/mounts` |
...
| 时间 | 命令 | 结果 | 备注 |
|------|------|------|------|
| 2026-06-20 | `e2b-sandbox-cleanup.sh` | OK 12/12 killed | 清 orphan |
| 2026-06-20 | `verify-e2b-ovs-e2e` (cleanup=0) | FAIL F9 agent WS 400 | 根因：:3002 非 traffic proxy |
| 2026-06-20 | `verify-e2b-ovs-e2e` (full cleanup) | FAIL F9 `HTTP 200 OK`（非 101） | ttyd 改 :80；仍命中 F14 默认站 |
...
| 2026-06-20 | 部署 traffic + podman 端口映射 + gateway 重建 | **PASS** | `verify-e2b-ovs-e2e: OK`（OVS 200 + agent WS + terminal/stop） |
...
| 全链路 | `./deploy/stack/lib/verify-e2b-ovs-e2e.sh` | 绿 | **FAIL** `OVS URL hit e2b default site` |
...
- `claw-e2b-sandbox-client`：self-hosted create 带 `secure: false`；`traffic_url()` 辅助
- gateway：ttyd WS 支持 `X-Access-Token` / `?token=`
- `deploy/stack/lib/setup-e2b-traffic-proxy.sh`：一键部署 e2b traffic
- `verify-e2b-ovs-e2e.sh`：`CLAW_OVS_E2E_FAST=1` 避免 agent WS 等完整 claw 回合

**验收：**

```bash
./deploy/stack/lib/verify-e2b-ovs-e2e.sh
# verify-e2b-ovs-e2e: OK
```
