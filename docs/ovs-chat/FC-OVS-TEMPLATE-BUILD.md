...
在自建 e2b（`CLAW_OVS_BACKEND=e2b`）上提供 **1 Gateway : 1 OVS** 单例沙箱：
...
| **禁止绕过硬编码模板 ID** | 运行时只用别名 `CLAW_E2B_OVS_TEMPLATE=claw-ovs`；临时改 `tpl_*` 不算修复 |

构建命令（repo 根，`.env` 已设 `CLAW_E2B_*`）：

```bash
set -a && source .env && set +a
# 可选：.venv-fc 含 e2b SDK（e2b-tap-live-up 同款）
python3 deploy/e2b/build-claw-ovs-selfhosted.py
```
...
| `CLAW_E2B_OVS_TEMPLATE` | **`claw-ovs`**（e2b 别名） |
| `CLAW_E2B_API_URL` / `CLAW_E2B_API_KEY` / `CLAW_E2B_DOMAIN` | 自建 e2b API |
| `CLAW_CONTAINER_RUNTIME` | `podman`（staging 用） |

e2b OVS 模板走本文 + `build-claw-ovs-selfhosted.py`；项目运行依赖 e2b 组件。

---

## 6. 运行时备注（Gateway 探活）

启动脚本：`rust/crates/http-gateway-rs/src/pool/interactive_backend/e2b_interactive_materialize.rs` → `start_ovs_server_sh`。
...
# 2) 能建沙箱
curl -sS -H "X-API-Key: $CLAW_E2B_API_KEY" -H 'Content-Type: application/json' \
...
通过标准：`ovs/workspace` 返回 `ovsUrl` + `ovsFolderUrl`；浏览器可开 folder；`verify-e2b-ovs-e2e.sh` 无 timeout。
...
| [e2b-nas-workspace.md](../e2b-nas-workspace.md) | NAS 路径契约 |
| `deploy/e2b/README.md` | e2b 交互总览 + 本模板构建入口 |
| `deploy/e2b/build-claw-ovs-selfhosted.py` | 构建脚本实现 |
