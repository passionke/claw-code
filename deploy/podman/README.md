...
**不想走 GitHub CI**：本机 **`linux/amd64`** 构建并直推私库（与 CI 同名 **`claw-code`** / **`claw-gateway-worker`**）：可先设 **`ACR_USER`（或 `ACR_USERNAME`）+ `ACR_PASSWORD`**，脚本会在 push 前对 **前缀里第一个 `/` 之前的主机名** 做非交互 `login`；或先手动 **`podman login <实例域名>`**。再 **`./deploy/podman/push-acr-local.sh crpi-xxx.cn-hangzhou.personal.cr.aliyuncs.com/<命名空间> release-v…`**；默认跨架构为 `linux/amd64`，可用 **`CLAW_PUSH_PLATFORM`** 覆盖。仅构建不推送：**`CLAW_LOCAL_PUSH_DRY_RUN=1`**。
...
网关 compose **只加载仓库根**的 `.env`。`CLAW_ALLOWED_TOOLS`（solve 工具白名单）只在该文件中配置；不设或留空表示允许全部内置工具。`deploy/podman/` 下由 `up.sh` / `compose-include` 生成的 `*.env` 为**中间物**，每次脚本会覆盖，**不要手改**。
