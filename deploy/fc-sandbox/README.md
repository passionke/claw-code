...
### Step C — FC interactive（方案 A：NAS 注入 claw/ttyd，无需自定义 template）

OVS `@claw` 需要沙箱内有 **`claw`** 与 **`ttyd`**。因 FC builder / ACR EE 路径不可行，采用 **官方 `code-interpreter-v1` + NAS 启动时拷贝二进制**。
...
产物：`{work_root}/.claw-fc-tools/claw` + `ttyd`（默认 `CLAW_FC_NAS_TOOLS_REL=.claw-fc-tools`）。

#### 2. `.env`（交互 FC 模式）

```bash
CLAW_INTERACTIVE_BACKEND=fc
CLAW_FC_TEMPLATE=code-interpreter-v1
NAS_BASE_URL=xxx.cn-beijing.nas.aliyuncs.com   # 或 CLAW_FC_NAS_SERVER
CLAW_FC_NAS_EXPORT=/                         # NAS 上 workspace 根，234 为 /
CLAW_FC_NAS_TOOLS_REL=.claw-fc-tools
...
`.env` 改用自建模板名，例如 `CLAW_FC_TEMPLATE=claw-fc-vpc-v1`。

#### 3. 运行时行为

每次 FC 沙箱 `terminal/start` 或 solve，`fc_exec.py` 在脚本前执行 `fc-nas-bootstrap.sh`：从 NAS 拷到 `/claw_host_root/.claw/bin` 并加入 `PATH`，再启动 ttyd / `claw gateway-solve-once`。
...
### 推荐路径：`builder` + 杭州 ACR worker 镜像（**当前被 FC 地域/仓库设计卡住**）
...
| **指定** 杭州 ACR 为 dest | FC builder 平面访问 `cr.cn-hangzhou.aliyuncs.com` **超时** |
...
（待实现：gateway FC 启动脚本 + NAS 一次性上传工具二进制。）

#### 路径 B — 自建 **北京 ACR EE** + VPC（官方文档正路）

- 与 FC 同地域、绑 VPC/vSwitch
...
export CLAW_FC_TEMPLATE=claw-worker-v1-prod
export CLAW_FC_TEMPLATE_BUILD_STRATEGY=from_image
export CLAW_FC_TEMPLATE_BUILD_MODE=builder
export CLAW_FC_TEMPLATE_SOURCE_REGISTRY_TYPE=acr
export CLAW_FC_TEMPLATE_FROM_IMAGE=crpi-cf9vxpq3n8or17mw.cn-hangzhou.personal.cr.aliyuncs.com/passionke/claw-gateway-worker:release-v1.6.12
export CLAW_FC_TEMPLATE_DEST_IMAGE_REF=fc-e2b-registry.cn-beijing.cr.aliyuncs.com/passionke/claw-worker:release-v1.6.12
export CLAW_FC_TEMPLATE_SKIP_CACHE=0

./deploy/fc-sandbox/build-claw-worker-template.sh
```
...
| `CLAW_FC_TEMPLATE` | **`code-interpreter-v1`**（方案 A 默认） |
| `CLAW_FC_NAS_TOOLS_REL` | NAS 上工具目录名，默认 `.claw-fc-tools` |
| `CLAW_USE_NAS_VOLUME` | `auto`（有 `NAS_BASE_URL` 即 compose NFS 直挂） |
| `CLAW_FC_NAS_EXPORT` | NAS export 子路径（234 多为 `/`） |
| `CLAW_FC_NAS_VOLUME_NAME` | **可选/legacy**：仅自建 template + 控制台预注册 volume 时用 |
...
| Variable | Role |
| --- | --- |
| `CLAW_FC_TEMPLATE_BUILD_STRATEGY` | `from_image` (default) or `dockerfile` |
| `CLAW_FC_TEMPLATE_BUILD_MODE` | `builder` (default) or `direct` |
| `CLAW_FC_TEMPLATE_FROM_IMAGE` | Source worker image |
| `CLAW_FC_TEMPLATE_DEST_*` | 仅 **自建北京 ACR EE** 时需要；**不是** fc-e2b-registry |
...
Requires `CLAW_INTERACTIVE_BACKEND=fc`, NAS tools installed, gateway up, and LLM configured for full OVS chat.
