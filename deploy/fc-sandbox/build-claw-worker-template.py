#!/usr/bin/env python3
# Build FC cloud sandbox template claw-worker-v1 (E2B SDK). Author: kejiqing
"""Build claw-worker-v1: FC Beijing base + claw/ttyd from local worker image."""
...
def _conn_opts() -> dict[str, str]:
    api_key = _env("E2B_API_KEY") or _env("CLAW_FC_API_KEY") or _env("ALIYUN_E2B_TOKEN")
    if not api_key:
        print("Set ALIYUN_E2B_TOKEN / CLAW_FC_API_KEY / E2B_API_KEY", file=sys.stderr)
        sys.exit(1)
    return {
        "api_key": api_key,
...
"""
    ...
    build = Template.build(
        ...
        cpu_count=cpu,
        memory_mb=mem,
        skip_cache=_env("CLAW_FC_TEMPLATE_SKIP_CACHE", default_skip_cache)
        not in ("0", "false", "no"),
...
def _build_from_image(
    ...
    from_image = _env("CLAW_FC_TEMPLATE_FROM_IMAGE")
    if not from_image:
        print("CLAW_FC_TEMPLATE_FROM_IMAGE required for from_image mode", file=sys.stderr)
        sys.exit(1)
    headers = _build_headers()
...
def main() -> int:
    ...
    if mode == "dockerfile":
        ...
        print(f"unknown CLAW_FC_TEMPLATE_BUILD_STRATEGY={mode!r}", file=sys.stderr)
        return 1
