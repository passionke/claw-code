#!/usr/bin/env python3
"""Persist gateway_global_settings from FC scripts via CLAW_GATEWAY_DATABASE_URL. Author: kejiqing"""
...
def database_url() -> str:
    for key in ("CLAW_FC_WORKER_DATABASE_URL", "CLAW_GATEWAY_DATABASE_URL"):
        ...
        if val:
            return val
    raise RuntimeError("set CLAW_GATEWAY_DATABASE_URL (or CLAW_FC_WORKER_DATABASE_URL) in .env")
...
def merge_settings_json_key(key: str, patch: dict[str, Any], *, now_ms: int | None = None) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "psycopg missing in FC venv; re-run script (venv install) or: pip install 'psycopg[binary]'"
        ) from exc
