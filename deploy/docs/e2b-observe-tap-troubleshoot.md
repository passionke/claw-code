...
- **创建 envVars：** `CLAW_CLUSTER_ID` + `CLAW_GATEWAY_DATABASE_URL` + **`CLAW_TAP_CLIENT`**（gateway 按 active LLM `baseModelUrl` **path** 推断，取值必须是 claude-tap `ClientName`：`/messages`→`claude`，`/responses`、`/chat/completions` 或裸 `/v1`→`codex`）
- **模板 start：** `claw-observe-start` 读 `--tap-client "${CLAW_TAP_CLIENT:-codex}"`（**发版后各环境重建一次 observe 模板**即可；之后换协议靠 Apply/reset，不必再为协议改模板）
