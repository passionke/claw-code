#!/usr/bin/env python3
"""Ensure e2b claw-nas-api singleton sandbox exists on e2b — gateway NAS read/write service.
...
Usage (repo root, `.env` with self-hosted e2b vars):
  python3 deploy/e2b/e2b-nas-api-up.py
  python3 deploy/e2b/e2b-nas-api-up.py --reuse
  python3 deploy/e2b/e2b-nas-api-up.py --reset
  python3 deploy/e2b/e2b-nas-api-up.py --json
...
def _fc_python() -> Path:
    ...
    py = venv / "bin" / "python3"
    if not py.is_file():
        print(f"==> create e2b venv {venv} (e2b SDK + psycopg)", file=sys.stderr)
...
def _nas_api_port() -> int:
    try:
        return int(_env("CLAW_E2B_NAS_API_PORT", "8090") or "8090")
    except ValueError:
        return 8090
...
def _persist_e2b_nas_api_to_pg(base_url: str, sandbox_id: str) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Start e2b e2b claw-nas-api singleton (gateway NAS service)")
    ...
    parser.add_argument("--no-persist", action="store_true", help="skip writing e2bNasApi to PG")
    ...
    if not api_key:
        print("error: set CLAW_E2B_API_KEY in .env", file=sys.stderr)
        return 1
    ...
    template = _env("CLAW_E2B_NAS_API_TEMPLATE") or "claw-nas-api"
    ...
    if not args.no_persist:
        print("==> persist e2bNasApi to PG …", file=sys.stderr)
        _persist_e2b_nas_api_to_pg(base_url, sandbox_id)
