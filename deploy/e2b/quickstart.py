#!/usr/bin/env python3
# Phase 0 e2b connectivity quickstart (Step A). Author: kejiqing
"""Minimal e2b sandbox smoke test — no NAS, built-in template."""
...
def main() -> int:
    api_key = os.environ.get("E2B_API_KEY") or os.environ.get("CLAW_E2B_API_KEY") or os.environ.get(
        "ALIYUN_E2B_TOKEN"
    )
    if not api_key:
        print("Set E2B_API_KEY, CLAW_E2B_API_KEY, or ALIYUN_E2B_TOKEN", file=sys.stderr)
        return 1

    domain = os.environ.get("E2B_DOMAIN", "cn-beijing.e2b.fc.aliyuncs.com")
    api_url = os.environ.get("E2B_API_URL") or os.environ.get("CLAW_E2B_API_URL")
    sandbox_url = os.environ.get("E2B_SANDBOX_URL") or os.environ.get("CLAW_E2B_SANDBOX_URL")
    template = os.environ.get("CLAW_E2B_TEMPLATE", "code-interpreter-v1")
