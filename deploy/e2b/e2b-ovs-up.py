#!/usr/bin/env python3
"""Ensure e2b OVS singleton sandbox exists on e2b — direct traffic URL, no gateway.
...
Usage (repo root, `.env` with self-hosted e2b vars):
  python3 deploy/e2b/e2b-ovs-up.py
  python3 deploy/e2b/e2b-ovs-up.py --reuse
  python3 deploy/e2b/e2b-ovs-up.py --reset
  python3 deploy/e2b/e2b-ovs-up.py --json
...
def _persist_e2b_ovs_to_pg(urls: dict[str, str]) -> None:
    ...
    merge_settings_json_key("e2bOvs", patch, now_ms=now_ms)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Start e2b e2b OVS singleton (openvscode-server)")
    ...
    parser.add_argument("--no-persist", action="store_true", help="skip writing e2bOvs to PG")
    parser.add_argument(
        "--proj-id",
        type=int,
        default=int(_env("CLAW_E2B_E2E_PROJ_ID", "3") or "3"),
    ...
    template = _env("CLAW_E2B_OVS_TEMPLATE") or "claw-ovs"
    ...
    if not args.no_persist:
        print("==> persist e2bOvs to PG …", file=sys.stderr)
        _persist_e2b_ovs_to_pg(result)
