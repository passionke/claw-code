...
RUN chmod +x /usr/local/bin/claude-tap \\
    && printf '%s\\n' \\
        ...
        'tap_client="${{CLAW_TAP_CLIENT:-openai}}"' \\
        'exec env CLAW_CLUSTER_ID="$CLAW_CLUSTER_ID" CLAW_GATEWAY_DATABASE_URL="$CLAW_GATEWAY_DATABASE_URL" CLAW_TAP_CLIENT="$tap_client" /usr/local/bin/claude-tap --tap-no-launch --tap-live --tap-host 0.0.0.0 --tap-port 8080 --tap-live-port "$live_port" --tap-client "$tap_client" --tap-target https://bootstrap.invalid/v1 --tap-output-dir /claw_ws/tap-traces --tap-no-update-check --tap-no-auto-update' \\
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"==> persisted e2bObserve.templateId={build.template_id!r} "
