#!/usr/bin/env python3
"""Runtime install claw-vscode into e2b OVS singleton (openvscode --install-extension).
...
Usage (repo root, .env loaded):
  python3 deploy/e2b/e2b-ovs-install-claw-vscode.py
  python3 deploy/e2b/e2b-ovs-install-claw-vscode.py --reuse
  python3 deploy/e2b/e2b-ovs-install-claw-vscode.py --json
...
EXEC_HELPER = ROOT / "deploy/e2b/e2b_exec.py"
...
def _fc_python() -> Path:
    ...
    py = venv / "bin" / "python3"
    if not py.is_file():
        print(f"==> create e2b venv {venv} (e2b SDK for fc_exec)", file=sys.stderr)
...
def _resolve_fc_ovs_gateway_host(gateway_port: int) -> str:
    ...
    explicit = _env("CLAW_E2B_OVS_GATEWAY_HOST")
    if explicit:
        return explicit if ":" in explicit else f"{explicit}:{gateway_port}"
    advertise = _env("CLAW_POOL_ADVERTISE_HOST") or _env("CLAW_E2B_GATEWAY_ADVERTISE_HOST")
    if advertise:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Runtime install claw-vscode in e2b OVS singleton")
    parser.add_argument("--reuse", action="store_true", help="reuse existing ovs-singleton (default)")
    parser.add_argument("--proj-id", type=int, default=int(_env("CLAW_E2B_E2E_PROJ_ID", "3") or "3"))
    ...
    print(f"==> e2b OVS claw.gatewayHost={gateway_host} (Remote EH → gateway agent/ws)", file=sys.stderr)
