...
claw_export_pool_registry_env() {
  local script_dir="${1:-}"
  local host_pinned=0
  if [[ -n "${CLAW_POOL_ADVERTISE_HOST:-}" || -n "${CLAW_POOL_ADVERTISE_IP:-}" ]]; then
    host_pinned=1
  fi
  export CLAW_POOL_ADVERTISE_HOST="$(claw_detect_pool_advertise_host)"
  export CLAW_POOL_ID="$(claw_default_pool_id)"
  export CLAW_POOL_ADVERTISE_HOST_PINNED="${host_pinned}"
  claw_sync_host_gateway_urls
  if [[ -n "${script_dir}" ]]; then
    mkdir -p "${script_dir}"
    {
      ...
      printf '%s\n' "CLAW_POOL_ADVERTISE_HOST_PINNED=${host_pinned}"
