#!/usr/bin/env bash
# Runtime install claw-vscode into FC OVS singleton (NAS VSIX + fc_exec). Author: kejiqing
set -euo pipefail
...
exec python3 "${REPO_ROOT}/deploy/fc-sandbox/fc-ovs-install-claw-vscode.py" "$@"
