...
API_KEY="${CLAW_E2B_API_KEY:-${ALIYUN_E2B_TOKEN:-}}"

fail() { echo "e2b-sandbox-cleanup: $*" >&2; exit 1; }

[[ -n "${API_KEY}" ]] || fail "CLAW_E2B_API_KEY or ALIYUN_E2B_TOKEN required"
...
echo "e2b-sandbox-cleanup: ${#ids[@]} sandbox(es) on ${API_URL}"
if ((${#ids[@]} == 0)); then
  echo "e2b-sandbox-cleanup: OK (nothing to kill)"
  exit 0
fi
...
echo "e2b-sandbox-cleanup: OK (killed ${killed}/${#ids[@]})"
