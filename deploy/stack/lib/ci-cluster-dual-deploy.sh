...
CLUSTER_A="${CLAW_CLUSTER_ID:?CLAW_CLUSTER_ID from node A .env}"
# Same-host CI: one clawTap → one CLAW_CLUSTER_ID; node B differs by pool/gateway only. kejiqing
...
echo "    node B cluster=${CLUSTER_A} (same clawTap as A) pool=${POOL_B}" >&2
...
CLAW_HOST_LOG_DIR=./claw-logs-ci-b
