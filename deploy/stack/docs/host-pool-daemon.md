...
GitLab CI（10.22.28.94）与 **GitHub CI（62.72.45.75）** 均设 **`CLAW_POOL_DAEMON_USE_SYSTEMD=0`**。GitHub 另设 **`CLAW_POOL_SUPERVISOR=docker`**：`claw-sandbox` 跑在 `docker run --restart unless-stopped` 容器里，避免 Actions job 结束时的 `Terminate orphan process … (claw-sandbox)`。生产 Linux 预发默认 systemd，见下文。
