...
> **2026-06：** 宿主机 pool / systemd `claw-sandbox` 已移除。新部署按 [`docs/architecture-governance.md`](../../../docs/architecture-governance.md) 验收 FC + 外连 PG。下文部分检查项为历史集群 pool 口径。
...
`CLAW_POOL_DAEMON_USE_SYSTEMD=0` 在旧 CI 有意为之；**FC-only 栈不再使用 pool systemd**。见 `docs/deploy-ops-truth.md`。
