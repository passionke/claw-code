...
export const RESTRICTION_MSG =
  "本服务仅允许 SELECT、SET、EXPLAIN、SHOW（可多条 SET 后跟一条 SELECT/EXPLAIN/SHOW，SET 不计数）；禁止 INSERT/UPDATE/DELETE/DROP/CREATE/ALTER 等写操作与 DDL。";
...
export function wrapSqlError(category: string, rawMessage?: string): string {
...
export function wrapExecutionError(_dorisMessage?: string): string {
...
export const ONLY_READONLY_MCP =
  "本服务为只读 Doris 查询 MCP，仅支持 SELECT、SET、EXPLAIN、SHOW；具体错误信息仅供参考，参考作用有限。";

const CONNECTION_ERROR_PATTERN =
  /closed state|Can't add new command when connection|Connection lost|fatal error|Cannot enqueue|offset out of range/i;
