...
function runPythonValidator(sql: string): {
  valid: boolean;
  reason?: string;
  detail?: string;
  stmt_type?: string;
...
export function validateDorisSql(sql: string): ValidationResult | ValidationError {
  ...
  switch (reason) {
    ...
    case "PARSE_ERROR":
      return {
        ok: false,
        message: wrapSqlError(
          "SQL 解析失败，请检查是否为合法 Doris/MySQL 语法。本服务仅支持单条 SELECT、SET、EXPLAIN 或 SHOW。",
          detail
        ),
      };
    case "PARSE_EMPTY":
