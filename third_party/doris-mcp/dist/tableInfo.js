...
export async function getTableMeta(conn, database, table) {
    ...
    return {
        ...
        tableRows: tableRows === null || tableRows === undefined
            ? null
...
export async function getColumnMeta(conn, database, table) {
...
export function buildTableInformationText(table, columns) {
    const lines = [];
    if (table) {
        ...
        if (table.tableComment)
            lines.push(`注释: ${table.tableComment}`);
        if (table.tableRows != null)
            lines.push(`行数(估): ${table.tableRows}`);
        if (table.createTime)
            lines.push(`创建时间: ${table.createTime}`);
        if (table.updateTime)
            lines.push(`更新时间: ${table.updateTime}`);
        lines.push("");
    }
    if (columns.length > 0) {
        lines.push("列信息:");
        const colLines = columns.map((c) => `  ${c.field}  ${c.type}  ${c.null}  ${c.key}  default=${c.default ?? "NULL"}  ${c.extra}  ${c.comment ?? ""}`);
        lines.push(...colLines);
    }
    return lines.join("\n");
}
