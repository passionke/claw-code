...
if [[ -z "${CLAW_GATEWAY_TEST_DATABASE_URL:-}" && -z "${CLAW_GATEWAY_DATABASE_URL:-}" ]]; then
  echo "==> [3/4] skip PG integration — set CLAW_GATEWAY_TEST_DATABASE_URL (e.g. postgres on :5433)"
