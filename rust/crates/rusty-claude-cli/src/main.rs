...
fn parse_gateway_interactive_once(args: &[String]) -> Result<CliAction, String> {
    ...
    let session_jsonl = session_jsonl.ok_or_else(|| {
        "gateway-interactive-once requires --session-jsonl PATH".to_string()
    })?;
    let prompt_b64 = prompt_b64
        .ok_or_else(|| "gateway-interactive-once requires --prompt-b64".to_string())?;
