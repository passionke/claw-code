...
pub fn build_claude_tap_health(request_host: Option<&str>) -> ClaudeTapHealthSnapshot {
    let proxy_port = env_u16(
        &["CLAUDE_TAP_HOST_PORT", "CLAUDE_TAP_PORT"],
        DEFAULT_TAP_PROXY_PORT,
    );
    ...
    let gateway_public = env_nonempty("CLAW_GATEWAY_PUBLIC_BASE_URL")
        .map(|s| expand_compose_gateway_port_template(&s));
    ...
    let live_template = format!("{public_live}/?{LIVE_SESSION_QUERY_PARAM}={{sessionId}}");
...
fn resolve_public_origin(
    ...
    let hostname =
        if fallback_host == "host.docker.internal" || fallback_host == "host.containers.internal" {
            "127.0.0.1".to_string()
        } else {
            fallback_host
        };
...
fn hostname_from_authority(authority: &str) -> Option<String> {
    ...
    let hostname = hostname.trim_matches(|c| c == '[' || c == ']').to_string();
...
mod tests {
    ...
    fn derives_live_from_gateway_public_base() {
        let snap =
            build_claude_tap_health_for_test(Some("http://192.168.9.252:18088"), None, 8080, 3000);
    ...
    fn expand_compose_gateway_port_placeholder() {
        ...
        let out =
            expand_compose_gateway_port_template("http://127.0.0.1:${GATEWAY_HOST_PORT:-18088}}");
    ...
    fn localhost_in_public_base_and_host_header() {
        let from_base =
            build_claude_tap_health_for_test(Some("http://localhost:18088"), None, 8080, 3000);
        ...
        let loopback =
            build_claude_tap_health_for_test(Some("http://127.0.0.1:8088"), None, 8080, 3000);
