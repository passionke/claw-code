...
#[must_use]
pub fn should_use_fc_traffic_browser_proxy(internal_base: &str) -> bool {
...
pub fn should_use_fc_observe_browser_proxy(internal_live_base: &str) -> bool {
    should_use_fc_traffic_browser_proxy(internal_live_base)
}
...
#[must_use]
pub fn fc_traffic_browser_hosts_line(
    _internal_base: &str,
    _fc_domain: &str,
...
pub fn fc_ovs_browser_hosts_line(
    ...
    fc_traffic_browser_hosts_line(internal_ovs_base, fc_domain)
}

/// Traffic proxy listen port on e2b host (nginx → :3001); gateway internal probes only.
#[must_use]
pub fn fc_traffic_proxy_port() -> u16 {
    std::env::var("CLAW_FC_TRAFFIC_PORT")
        .ok()
...
pub fn self_hosted_observe_upstream(
    ...
    if domain.is_empty() {
        return Err("CLAW_FC_DOMAIN empty".into());
    }
    let port = fc_traffic_proxy_port();
    Ok((traffic_host, format!("http://{domain}:{port}")))
}

/// FC observe singleton: Admin Live URLs are SDK Host URLs (no path translation).
#[must_use]
