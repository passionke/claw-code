//! FC observe tap reset — delegate lifecycle to `fc-tap-live-up.py`. Author: kejiqing
...
use crate::pool::interactive_backend::fc_observe_is_enabled;
...
fn resolve_fc_tap_live_up_script() -> Result<PathBuf, String> {
    let repo = pool_worker_runtime_sync::resolve_repo_root()
        .ok_or_else(|| "CLAW_REPO_ROOT unset; cannot run fc-tap-live-up.py".to_string())?;
    let script = repo.join("deploy/fc-sandbox/fc-tap-live-up.py");
    if !script.is_file() {
...
async fn run_fc_tap_live_up_reset() -> Result<FcTapLiveUpJson, String> {
    ...
    let repo = script
        ...
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
        .ok_or_else(|| "invalid fc-tap-live-up.py path".to_string())?;
    let output = Command::new("python3")
        ...
        .output()
        .await
        .map_err(|e| format!("fc-tap-live-up --reset: {e}"))?;
    ...
    if !output.status.success() {
        ...
        return Err(format!(
            "fc-tap-live-up --reset exit {}: {}",
            output.status.code().unwrap_or(-1),
            detail
        ));
    }
    serde_json::from_str(stdout.trim())
        .map_err(|e| format!("parse fc-tap-live-up JSON: {e}: {stdout}"))
}

pub async fn reset_observe_tap(db: &GatewaySessionDb) -> Result<ObserveTapResetResponse, String> {
    if !fc_observe_is_enabled() {
        return Err(
            "FC observe tap disabled (CLAW_INTERACTIVE_BACKEND≠fc or CLAW_FC_OBSERVE=0)".into(),
        );
    }
