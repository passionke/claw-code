//! e2b worker co-located claude-tap **proxy** (`127.0.0.1:8080`): LLM 代理 + session trace **写入**。
...
use base64::Engine;
use claw_e2b_sandbox_client::GUEST_CLAW_TAP_TRACES;

pub const E2B_WORKER_TAP_PROXY_URL: &str = "http://127.0.0.1:8080";
const FC_WORKER_TAP_PORT: u16 = 8080;

/// Worker LLM env must hit co-located tap, not compose `claw-claude-tap`.
#[must_use]
pub fn e2b_worker_llm_env(mut env: BTreeMap<String, String>) -> BTreeMap<String, String> {
    env.insert(
        "OPENAI_BASE_URL".to_string(),
        E2B_WORKER_TAP_PROXY_URL.to_string(),
    );
    env.insert(
...
/// PG URL reachable from e2b sandbox (VPN). Falls back to gateway env when unset.
pub fn e2b_worker_tap_database_url() -> Result<String, String> {
    if let Ok(raw) = std::env::var("CLAW_E2B_WORKER_DATABASE_URL") {
...
#[must_use]
pub fn build_e2b_worker_tap_start_script(
    cluster_id: &str,
    db_url: &str,
    upstream_target: &str,
) -> String {
    ...
    format!(
        r#"set -e
TAP_BIN=""
for cand in /usr/local/bin/claude-tap /tmp/claw-e2b-bin/claude-tap /tmp/claw-e2b-bin/tap-runtime/bin/claude-tap; do
...
pub async fn build_e2b_worker_tap_start_script_from_db(
    session_db: &GatewaySessionDb,
) -> Result<String, String> {
    let cluster_id = gateway_cluster_id()?;
    let db_url = e2b_worker_tap_database_url()?;
    ...
    Ok(build_e2b_worker_tap_start_script(
        &cluster_id,
        &db_url,
        &upstream,
    ))
...
pub async fn build_e2b_session_attach_with_tap(
    session_db: &GatewaySessionDb,
    llm_env: &BTreeMap<String, String>,
) -> Result<String, String> {
    let tap = build_e2b_worker_tap_start_script_from_db(session_db).await?;
...
#[must_use]
pub fn e2b_worker_solve_route(mut route: SolveLlmRoute) -> SolveLlmRoute {
    route.claw_tap_base_url = Some(E2B_WORKER_TAP_PROXY_URL.to_string());
    route
}
...
pub async fn resolve_e2b_worker_solve_llm_route(
    ...
    let route = e2b_worker_solve_route(SolveLlmRoute {
        mode: "e2bWorkerTap".to_string(),
        cluster_id: cluster_id.clone(),
        cluster_hash: local.cluster_hash.clone(),
        claw_tap_base_url: Some(E2B_WORKER_TAP_PROXY_URL.to_string()),
    ...
    Ok((route, e2b_worker_llm_env(env)))
}
...
mod tests {
    ...
    #[test]
    fn e2b_worker_llm_env_points_at_localhost_tap() {
        let mut env = BTreeMap::new();
        env.insert(
            "OPENAI_BASE_URL".to_string(),
            "http://claw-claude-tap:8080".to_string(),
        );
        let out = e2b_worker_llm_env(env);
        assert_eq!(
            out.get("OPENAI_BASE_URL").map(String::as_str),
            Some(E2B_WORKER_TAP_PROXY_URL)
        );
    }

    #[test]
    fn tap_start_script_mentions_local_healthz() {
        let sh = build_e2b_worker_tap_start_script(
            "local-dev",
