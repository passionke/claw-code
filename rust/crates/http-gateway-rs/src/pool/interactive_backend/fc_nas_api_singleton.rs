//! FC NAS API client — gateway NAS layout + file writes via the out-of-band
...
use crate::gateway_fc_nas_api_settings::load_fc_nas_api_base_url;
...
impl Default for FcNasApiSingleton {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FcNasApiSingleton {
    ...
    async fn base_url(&self) -> Result<String, String> {
        ...
        let url = load_fc_nas_api_base_url(&db)
            .await
            .map_err(|e| format!("load fcNasApi endpoint from PG: {e}"))?;
        url.ok_or_else(|| {
    ...
    pub fn enabled_from_env() -> bool {
        match std::env::var("CLAW_FC_NAS_API")
            .ok()
