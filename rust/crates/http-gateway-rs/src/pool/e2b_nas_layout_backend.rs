//! e2b NAS layout + materialize via claw-nas-api singleton (required). Author: kejiqing
...
use super::interactive_backend::E2bNasApiSingleton;
...
pub struct NasLayoutBackend {
    nas_api: Arc<E2bNasApiSingleton>,
}

impl NasLayoutBackend {
    #[must_use]
    pub fn new(nas_api: Arc<E2bNasApiSingleton>) -> Self {
        Self { nas_api }
    }
    ...
    pub async fn replace_git_import_dest(
        ...
        proj_id: i64,
        dest_rel: &str,
        bundle: &crate::project_git_sync::GitImportBundle,
    ) -> Result<(), String> {
        ...
        tracing::info!(
            ...
            dest = %dest_abs,
            "git import upload to nas-api"
        );
        self.nas_api
            .upload_git_import_tar_gz_dest(&dest_abs, &bundle.tar_gz)
            .await
    }
    ...
    pub async fn prepare_e2b_worker_bind_sources(
        ...
        self.ensure_e2b_proj_nas_roots(proj_id).await?;
