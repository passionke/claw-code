//! FC claw-nas-api singleton endpoint persisted by `fc-nas-api-up.py` in PG. Author: kejiqing
//!
...
//! `gateway_global_settings.settings_json.fcNasApi`. The gateway is a pure
...
pub async fn load_fc_nas_api_settings(
    db: &GatewaySessionDb,
) -> Result<FcNasApiSettings, sqlx::Error> {
    let (settings, _, _) = get_gateway_global_settings(db).await?;
    Ok(settings.fc_nas_api)
}

pub async fn load_fc_nas_api_base_url(
    db: &GatewaySessionDb,
) -> Result<Option<String>, sqlx::Error> {
    let s = load_fc_nas_api_settings(db).await?;
    Ok(s.base_url
