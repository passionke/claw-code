//! e2b sandbox env config. Author: kejiqing

use std::path::PathBuf;

/// Resolved e2b / E2B API settings from environment.
#[derive(Debug, Clone)]
...
impl E2bSandboxConfig {
    ...
    pub fn from_env() -> Option<Self> {
        ...
        let api_key = std::env::var("CLAW_E2B_API_KEY")
            .ok()
        ...
        let api_url = std::env::var("CLAW_E2B_API_URL")
            .ok()
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| "https://api.cn-beijing.e2b.fc.aliyuncs.com".into());
        let domain = std::env::var("CLAW_E2B_DOMAIN")
            .ok()
        ...
        let sandbox_url = std::env::var("CLAW_E2B_SANDBOX_URL")
            ...
            .filter(|v| !v.trim().is_empty());
        let template = std::env::var("CLAW_E2B_TEMPLATE")
            .ok()
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| "code-interpreter-v1".into());
        let sandbox_timeout_secs = std::env::var("CLAW_E2B_SANDBOX_TIMEOUT_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(3600);
        let nas_server = std::env::var("CLAW_E2B_NAS_SERVER")
            .ok()
            .or_else(|| std::env::var("NAS_BASE_URL").ok())
            .filter(|v| !v.trim().is_empty());
        let nas_export = std::env::var("CLAW_E2B_NAS_EXPORT")
            .ok()
            .filter(|v| !v.trim().is_empty());
        ...
        let nas_group_id = std::env::var("CLAW_WORKER_GID")
            ...
            .and_then(|v| v.parse().ok())
            .unwrap_or(1000);
        let exec_helper = std::env::var("CLAW_E2B_EXEC_HELPER")
            .ok()
            .map(PathBuf::from)
    ...
    /// Self-hosted e2bserver (`passionke/e2bserver`) vs Alibaba e2b cloud sandbox.
    #[must_use]
...
fn default_exec_helper_path() -> PathBuf {
    PathBuf::from("deploy/e2b/e2b_exec.py")
}

fn warn_if_legacy_nas_volume_name() {
    if std::env::var("CLAW_E2B_NAS_VOLUME_NAME")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .is_some()
    {
        eprintln!(
            "claw-e2b-sandbox-client: CLAW_E2B_NAS_VOLUME_NAME is ignored; use nasConfig bind"
        );
    }
}
...
mod tests {
    ...
    fn default_exec_helper_relative_to_repo() {
        ...
        assert!(p.to_string_lossy().contains("e2b_exec.py"));
    }
}
