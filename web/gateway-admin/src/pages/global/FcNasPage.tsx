...
import type { FcNasSettings, GlobalSettingsResponse } from "../../types/globalSettings";

/** Admin read-only FC NAS view (source: repo `.env`, restart gateway to apply). Author: kejiqing */
export default function FcNasPage() {
  const { gatewayBase } = useApp();
  const [loading, setLoading] = useState(false);
  const [settings, setSettings] = useState<FcNasSettings | null>(null);
      ...
      <Alert
        ...
        description={
          <Typography.Paragraph style={{ marginBottom: 0 }}>
            ...
            <Typography.Text code>CLAW_FC_NAS_SERVER</Typography.Text>、
            <Typography.Text code>CLAW_FC_NAS_EXPORT</Typography.Text>、
      ...
      <Card title="当前环境变量（只读）" loading={loading}>
        {settings ? (
          <Descriptions column={1} bordered size="small">
            ...
            <Descriptions.Item label="CLAW_FC_NAS_SERVER">
              <Typography.Text code>{settings.fcNasServer || "（未设置）"}</Typography.Text>
            </Descriptions.Item>
            <Descriptions.Item label="CLAW_FC_NAS_EXPORT">
              <Typography.Text code>{settings.fcNasExport || "（未设置）"}</Typography.Text>
            </Descriptions.Item>
